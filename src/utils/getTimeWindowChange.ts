import { IArsArray } from "../../types/fetchPair";

const MIN_VALUE_DISPLAYED = 0.001;

export const getTimeWindowChange = (lineChartData: IArsArray) => {
  if (lineChartData.length > 0) {
    const firstValue =
      lineChartData.find(({ price }) => !!price && price > 0)?.price ?? 0;
    const lastValue = lineChartData[lineChartData.length - 1].price;
    const changeValue = lastValue - firstValue;

    return {
      changeValue:
        changeValue > 0
          ? Math.max(changeValue, MIN_VALUE_DISPLAYED)
          : Math.min(changeValue, MIN_VALUE_DISPLAYED * -1),
      changePercentage: ((changeValue / firstValue) * 100).toFixed(2)
    };
  }

  return {
    changeValue: 0,
    changePercentage: 0
  };
};
